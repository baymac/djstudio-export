...
sync/                           Faithful capture of source-app playlists (Apple Music / Spotify)
  capture.py                    `dj sync music|spotify`: playlists/library → sync_tracks + sync_playlist_tracks
  push.py                       `dj sync <app> playlist push`: selected sync_tracks → recreate an app playlist
  enrich_adapter.py             `dj enrich --sync` adapter: drives enrich/engine over sync_tracks
  db.py / cli.py                canonical track store + membership; argparse dispatch

enrich/                         Unified enrichment + DJ Studio analysis
  cli.py                        `dj enrich` (default both; --detect/--sync to scope) + `dj enrich analyse`
...
uv run dj_cli.py sync music --library                                                   # capture Apple Music → sync_tracks (faithful)
uv run dj_cli.py sync spotify                                                            # capture all Spotify playlists → sync_tracks
...
uv run dj_cli.py sync beatport                                                          # pull Beatport library → enriched_tracks
uv run dj_cli.py enrich                                                                 # enrich detected + synced tracks → enriched_tracks
uv run dj_cli.py enrich --detect                                                        #   only detected tracks (or --sync for synced)
...
- **Sync capture is a non-destructive split, mirroring the Beatport side** — `dj sync music|spotify` writes to two tables (`sync/db.py`): **`sync_tracks`** is the canonical per-`(app, dedup_key)` track store (native id when present, else normalised artist+title), append-only/upsert — a track row is **never** deleted by a re-sync, so a delete on the source app can't destroy captured data; enrich state (`enrich_outcome`, `enriched_beatport_id`) lives here once per unique track. **`sync_playlist_tracks`** holds playlist→track membership links + position. `replace_playlist` re-snapshots only a playlist's membership (positions rewritten, removed tracks lose their link but keep their canonical row); it returns `{new, kept, removed, total}` for the per-playlist `+N new, M skipped` log line. **There is intentionally no DB-side delete** — `dj.db` is the permanent backup. `dj sync <app> playlist delete` removes playlists from the **source app** (Apple Music via AppleScript `delete playlist`, Spotify via unfollow `DELETE /playlists/{id}/followers`, Beatport via `DELETE /my/playlists/{id}/`), never from our DB; it only deletes what's been captured, offers to **sync-first** so the backup is current, then asks **once** before the (irreversible) delete. Scopes are per-source: **music** `--playlists` = all playlists incl. "Favourite Songs" (the `__library__` grouping excluded), `--all` = that + `clear_apple_library()` (batched AppleScript wipe of `library playlist 1`); **spotify** `--playlists` = unfollow all playlists, `--all` = that + `clear_saved_tracks()` (batched `DELETE /me/tracks` over Liked Songs); **beatport** = `--all` only (no library/liked concept). The `__library__` pseudo-playlist (Apple Music library / Spotify Liked Songs) is never a playlist target — it's only touched by `--all`'s collection clear. For Apple Music, "Favourite Songs" can appear under both its `__favorites__` pseudo-id and its real streamed id, so playlist targets are **deduped by name**. This is the same shape as `enriched_tracks` + `beatport_playlist_tracks`. `init_db()` auto-migrates a pre-split flat `sync_tracks` (old `native_playlist_id`/`position` columns) on first run. The legacy `synced_tracks`/`sync_runs` tables (old genre-classifier outcomes) are gone — that flow was removed.
- **Apple Music playlist *restore* (`dj sync music playlist push`) matches by persistent id → name+artist+album, by macOS necessity** — on macOS MusicKit cannot write the library: `MusicLibrary.createPlaylist` AND `MusicLibrary.add` are both `@available(macOS, unavailable)` (verified by `swiftc -typecheck`), so the only write path is scripting Music.app, and AppleScript cannot reference a track by its Apple Music **catalog id** (a cloud track exposes only `name`/`artist`/`album`/`database ID`/`persistent ID` — no store id). So the stable identifier we use is the AppleScript **`persistent ID`**: MusicKit doesn't provide it, so capture reads it from Music.app (`musickit.read_playlist_persistent_ids`) and attaches it to each captured row position-by-position, but **only when the two enumerations agree exactly** (same length + same track name at every index; ambiguous duplicate-named playlists and any read error → unset). It lands in the additive nullable `sync_tracks.native_persistent_id` (COALESCE-preserved, never overwritten with NULL). `build_create_playlist_applescript` then matches per track in order: (1) exact `whose persistent ID is …` (collision-proof, survives metadata edits), (2) fallback `name + artist` with captured `album` as a tiebreaker when several library tracks collide. For a track that has **left the library**, name+artist/persistent-id can't match it — the only macOS path to put it back is the best-effort `itmss://` store-URL trick (see the restore bullet); otherwise it's dropped, surfaced via `added < requested`. Do not re-attempt a "match by catalog id in AppleScript" path — the catalog id isn't reachable from AppleScript.
- **Bulk restore is the inverse of capture (`dj sync music playlist push --all/--playlists/--library/--favorite-only`, `sync/restore.py`)** — rebuilds Apple Music from the `sync_tracks` backup, run in order library → playlists → favorites. **`--playlists`** recreates each captured user playlist via `create_apple_playlist` (matches tracks already in the library — the reversible round-trip for a `delete --playlists`). **`--library`** repopulates the library from `__library__` rows and **`--favorite-only`** re-marks `__favorites__` rows as `loved` (`musickit.mark_loved`); both reach tracks not in the library only through `musickit.readd_track_by_catalog_id` — the **best-effort, EXPERIMENTAL** `itmss://itunes.apple.com/song?id=<native_track_id>` open (the only macOS catalog re-add; MusicKit `add` is unavailable). **`--readd-missing`** drops tracks already in the library (matched by name+artist via `read_library_track_keys`, since a re-added track gets a *fresh* persistent id) so the re-add is idempotent and resumable (Ctrl-C, re-run). Region-locked/removed tracks never re-add — accepted. **Spotify & Beatport restore are exact** (real APIs add by id, no `itmss://` hack): **spotify** `--playlists` recreates each captured playlist (`create_playlist` + `add_tracks`), `--library` re-saves Liked Songs (`spotify.save_tracks` → `PUT /me/tracks`), `--all` = both (no `--favorite-only`/`--readd-missing` — Liked Songs *is* the library); **beatport** `--all`/`--playlists` (equivalent — no library/liked concept) recreate every `beatport_playlists` row on the account from `detect.db.beatport_track_ids_in_playlist` via `beatport.create_playlist` + `add_track`. `restore_music`/`restore_spotify`/`restore_beatport` all live in `sync/restore.py`; the legacy `helpers/{export,restore,clear}_apple_music.py` scripts are superseded by this + `playlist delete`. Restore only adds — nothing destructive on its own.
- **`--library` cursor** — `dj sync music --library` tracks the last `library_added_date` processed in the `cursors` table (key `apple_music_library`); re-runs only capture new Apple Music additions.
